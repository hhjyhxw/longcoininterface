...
public class DateUtil {
	...
	public static Date getPureDate(String source) {
		...
		try {
			...
			date = new Date(cl.getTime().getTime());
		} catch (Exception e) {
			date = null;
		}
	 ...
	 */
	public static String getYearMonthDayWithMinus(Date date) {
	 ...
	 */
	public static String getYearMonthDayWithMilsec(Date date) {
	...
	public static String commonFormatDate(Date date, String fmt) {
		if (null == date)
			return "";
	...
	public static int compareWithCurrentDate(Date tgtDate) {
		if (tgtDate == null)
			return -1;
		int[] arr = getCurrentDateArray();
		Date curDate = getDate(arr[0], arr[1], arr[2]);
		arr = getYMD(tgtDate);
	...
	public static Date[] getLastesMonths(Date date) {
		...
		for (int index = 0; index < dates.length; index++) {
			...
			dates[index] = new Date(cl.getTime().getTime());
		}
